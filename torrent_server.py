    ...
    def _do_handshake(self, connection_info: ConnectionInfo):
        ...
        if len(payload) != size:
            ...
            try:
                ...
                logging.debug(
                    f"Handshake message received from peer {handshake.peer_id}")
        ...
        while 1:
            if not connection_info.handshaked:
                if not self._do_handshake(connection_info):
                    logger.warning(
                        f"Handshake failed for {connection_info.address}")
                    # close connection
                    break
                else:
                    # send bitfield
                    connection_info.connection.send(BitfieldMessage(
                        self.piece_manager.bitfield).message())
                ...
                if msg:
                    ...
                    logger.debug(
                        f"{msg.name} received from {connection_info.address}")
                    ...
                    if isinstance(msg, RequestMessage):
                        if connection_info.peer_id not in self.downloading_pieces[msg.piece_index]:
                            self.downloading_pieces[msg.piece_index].add(
                                connection_info.peer_id)
                        block = self.piece_manager.get_block_piece(
                            msg.index, msg.begin, msg.length)
                        connection_info.connection.send(PieceMessage(
                            msg.index, msg.begin, block).message())
                    ...
                    logger.exception('Not is a valid message')
            if time.time() - connection_info.last_call > 20:
                logger.warning(
                    f"Connection closed by {connection_info.address}")
                connection_info.connection.send(
