...
def main():
                ...
                if host1_ip != client_ip:
                    with ChordConnection(host1_ip) as chord_conn1:
                        if chord_conn1:
                            ...
                            connections.append(
                                f"{host1_ip} --------------------> {server_ip}"
                            )
                            logger.info(
                                "Tracker with ip %s joined Tracker with ip %s",
                                host1_ip,
                                server_ip,
                            )
    ...
    for info_hash, value in zip(dummy_info_hashes, data):
        with ChordConnection(server_ip) as chord_conn:
            chord_conn.store_key(info_hash, value, 0, 0, False)
            time.sleep(5)
